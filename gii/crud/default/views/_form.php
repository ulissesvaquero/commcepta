...
foreach ($tableSchema->getColumnNames() as $attribute) {
    if (!in_array($attribute, $generator->skippedColumns)) {
        echo $attribute !== 'id' ? "    <div class='col-xd-12 col-sm-6 col-md-4'>\n" : '';
        echo "        <?= " . $generator->generateActiveField($attribute, $fk) . " ?>\n";
        echo $attribute !== 'id' ? "    </div>\n\n" : "\n";
    }
}
...
<?= "        <?= " ?>Html::submitButton($model->isNewRecord ? <?= $generator->generateString('Cadastrar') ?> : <?= $generator->generateString('Atualizar') ?>, ['class' => $model->isNewRecord ? 'btn btn-success' : 'btn btn-primary']) ?>
<?= "    <?php endif; ?>\n" ?>
<?= "    <?php if(Yii::\$app->controller->action->id != 'create'): ?>\n" ?>
<?= "        <?= " ?>Html::submitButton(<?=$generator->generateString('Salvar como novo')?>, ['class' => 'btn btn-info', 'value' => '1', 'name' => '_asnew']) ?>
<?= "    <?php endif; ?>\n" ?>
<?php else: ?>
<?= "        <?= " ?>Html::submitButton($model->isNewRecord ? <?= $generator->generateString('Cadastrar') ?> : <?= $generator->generateString('Atualizar') ?>, ['class' => $model->isNewRecord ? 'btn btn-success' : 'btn btn-primary']) ?>
<?php endif; ?>
